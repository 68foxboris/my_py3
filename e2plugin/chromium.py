...
class ChromiumOSSettings(ConfigListScreen, Screen):
    ...
    def makeConfigList(self):
        ...
        entryStartUp = (_("Start Up URL"), config.plugins.browser.startup)
        entryMarginX = (_("X-margin on GUI"), config.plugins.browser.margin_x)
        entryMarginY = (_("Y-margin on GUI"), config.plugins.browser.margin_y)
        entryEnableNtpd = (_("Enable Ntpd when start ChromiumOS"), config.plugins.browser.enable_ntpd)
        entryNtpdUri = (_("Ntpd URL"), config.plugins.browser.ntpd_url)
#               entryRcuType    = (_("RCU language type"), config.plugins.browser.rcu_type)
...
class ChromiumOSWindow(ConfigListScreen, Screen):
    ...
    def makeConfigList(self):
        self.menulist = []
        entryURL = (_("Start URL"), config.plugins.browser.startup)
#               entryRcuType = (_("RCU Type"), config.plugins.browser.rcu_type)
